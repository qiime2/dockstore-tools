...
id: qiime2_diversity_lib_beta_phylogenetic_meta_passthrough
requirements:
  EnvVarRequirement:
    envDef:
      LC_ALL: en_US.UTF-8
  ...
  InitialWorkDirRequirement:
    ...
    - entryname: inputs.json
      entry: '{"inputs": $(inputs)}'
label: Beta Phylogenetic Meta Passthrough
doc: Computes a distance matrix for all pairs of samples in the set of feature table
  and phylogeny pairs, using the unifrac implementation of the selected beta diversity
  metric.
inputs:
  tables:
    doc: The feature tables containing the samples over which beta diversity should
      be computed.
    type: File[]
  phylogenies:
    doc: Phylogenetic trees containing tip identifiers that correspond to the feature
  ...
  bypass_tips:
    ...
    type: boolean
  weights:
    doc: The weight applied to each tree/table pair. This tuple is expected to be
      in index order with tables and phylogenies. Default is to weight each tree/table
      pair evenly.
    type: double[]?
  consolidation:
    default: skipping_missing_values
    doc: The matrix consolidation method, which determines how the individual distance
