...
struct qiime2_dada2_denoise_ccs_params {
...
task qiime2_dada2_denoise_ccs {
    ...
    input {
        ...
        Float min_fold_parent_over_abundance = 3.5
    ...
    qiime2_dada2_denoise_ccs_params task_params = object {
    ...
    command {
        q2dataflow q2wdl run dada2 denoise_ccs ~{write_json(task_params)}
    }
...
workflow wkflw_qiime2_dada2_denoise_ccs {
input {
        File demultiplexed_seqs
        String front
        String adapter
    ...
    call qiime2_dada2_denoise_ccs {
        input: demultiplexed_seqs=demultiplexed_seqs, front=front, adapter=adapter, max_mismatch=max_mismatch, indels=indels, trunc_len=trunc_len, trim_left=trim_left, max_ee=max_ee, trunc_q=trunc_q, min_len=min_len, max_len=max_len, pooling_method=pooling_method, chimera_method=chimera_method, min_fold_parent_over_abundance=min_fold_parent_over_abundance, allow_one_off=allow_one_off, n_threads=n_threads, n_reads_learn=n_reads_learn, hashed_feature_ids=hashed_feature_ids, table=table, representative_sequences=representative_sequences, denoising_stats=denoising_stats
    }
