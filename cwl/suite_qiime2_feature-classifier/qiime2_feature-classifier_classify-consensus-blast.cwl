...
id: qiime2_feature_classifier_classify_consensus_blast
requirements:
  EnvVarRequirement:
    envDef:
      LC_ALL: en_US.UTF-8
  ...
  InitialWorkDirRequirement:
    ...
    - entryname: inputs.json
      entry: '{"inputs": $(inputs)}'
label: BLAST+ consensus taxonomy classifier
doc: Assign taxonomy to query sequences using BLAST+. Performs BLAST+ local alignment
  between query and reference_reads, then assigns consensus taxonomy to each query
  sequence from among maxaccepts hits, min_consensus of which share that taxonomic
  assignment. Note that maxaccepts selects the first N hits with > perc_identity similarity
  to query, not the top N matches. For top N hits, use classify-consensus-vsearch.
inputs:
  query:
    doc: Query sequences.
    type: File
