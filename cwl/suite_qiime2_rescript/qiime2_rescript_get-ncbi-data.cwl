...
id: qiime2_rescript_get_ncbi_data
requirements:
  EnvVarRequirement:
    envDef:
      LC_ALL: en_US.UTF-8
  ...
  InitialWorkDirRequirement:
    ...
    - entryname: inputs.json
      entry: '{"inputs": $(inputs)}'
label: Download, parse, and import NCBI sequences and taxonomies
doc: 'Download and import sequences from the NCBI Nucleotide database and download,
  parse, and import the corresponding taxonomies from the NCBI Taxonomy database.
...
inputs:
  query:
    doc: Query on the NCBI Nucleotide database
    type: string?
  q2cwl_metafile_accession_ids:
    doc: List of accession ids for sequences in the NCBI Nucleotide database.
    type:
    - File?
    - File[]?
  ranks:
  ...
  n_jobs:
    ...
    type: long
  sequences:
    doc: Sequences from the NCBI Nucleotide database
    type: string
  taxonomy:
