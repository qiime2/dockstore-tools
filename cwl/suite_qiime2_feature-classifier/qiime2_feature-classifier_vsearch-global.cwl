...
id: qiime2_feature_classifier_vsearch_global
requirements:
  EnvVarRequirement:
    envDef:
      LC_ALL: en_US.UTF-8
  ...
  InitialWorkDirRequirement:
    ...
    - entryname: inputs.json
      entry: '{"inputs": $(inputs)}'
label: VSEARCH global alignment search
doc: Search for top hits in a reference database via global alignment between the
  query sequences and reference database sequences using VSEARCH. Returns a report
  of the top M hits for each query (where M=maxaccepts or maxhits).
inputs:
  query:
    doc: Query Sequences.
    type: File
